        ...
        <div v-else class="container mx-auto p-4 pt-20">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-medium">Analisis Barang Keluar</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div class="bg-red-50 rounded-lg p-4">
                        <h3 class="text-sm text-red-600 mb-1">Total Barang Keluar</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- Logs Perubahan Barang Keluar -->
                <div class="bg-white rounded-lg shadow-sm h-[calc(100vh-28rem)]">
                    <div class="sticky top-0 bg-white p-4 border-b z-10">
                        <h2 class="text-lg font-medium">Logs Perubahan Barang Keluar</h2>
                    </div>
                    <div class="overflow-y-auto h-[calc(100%-4rem)]">
                        <div class="divide-y">
                            <div v-for="log in filteredOutgoingLogs" :key="log._id"
                                 class="p-3 hover:bg-gray-50 transition-colors">
                                <div class="flex justify-between items-start mb-2">
...
// Reactive state
const inventoryOut = ref<InventoryOutItem[]>([]);
...
const sortedInventoryOut = computed(() =>
	[...inventoryOut.value].sort(
		(a, b) => new Date(b.date_out).getTime() - new Date(a.date_out).getTime(),
	),
);

const filteredOutgoingLogs = computed(() =>
	[...stockChangeLogs.value]
		.filter((log) => log.change_type === "Pengurangan")
		.sort(
			(a, b) =>
...
const filteredData = computed(() => {
	...
	return inventoryOut.value.filter(
		(item) => new Date(item.date_out) >= cutoffDate,
	);
});
...
const groupDataByDate = () => {
	...
	for (const item of filteredData.value) {
		const date = new Date(item.date_out).toISOString().split("T")[0];
		if (data[date]) {
...
const getInventoryOut = async () => {
	try {
		const response = await axios.get<ApiResponse<InventoryOutItem>>(
			`${import.meta.env.VITE_BACKEND_PORT}/inventory/out`,
		);
		if (response.data.invenOut) {
		...
		if (err.response?.status === 401) {
			...
			errorMessage.value = "Gagal memuat data barang keluar";
		}
		console.error("Error fetching inventory out:", err.message);
	}
};
...
const getStockChangeLogs = async () => {
	try {
		...
		const err = error as ApiError;
		console.error("Error fetching stock change logs:", err.message);
